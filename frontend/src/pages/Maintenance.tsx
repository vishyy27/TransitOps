          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-cream-light/90 text-sm">
              {filteredRecords.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map((record) => {
                const vehicle = state.vehicles.find(v => v.id === record.vehicleId);
                return (
                  <tr key={record.id} className="hover:bg-cream-light/35 transition-colors">
                    <td className="py-4 px-5 whitespace-nowrap text-ink font-medium">{format(new Date(record.date), 'MMM d, yyyy')}</td>
                    ...
                    <td className="py-4 px-5 text-ink/90 max-w-xs truncate font-medium">{record.description}</td>
                    <td className="py-4 px-5 font-semibold text-ink font-mono">${record.cost.toLocaleString()}</td>
                    <td className="py-4 px-5">
              ...
              {filteredRecords.length === 0 && (
                <tr>
                  <td colSpan={7} className="py-10 text-center text-sm font-semibold text-rust/80">No maintenance logs found.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-cream">
        <div className="flex justify-between items-center p-5 border-b border-cream/50 bg-cream-light/60">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-lg font-bold font-display text-ink">New Maintenance Log</h2>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 text-sm font-medium">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-ink uppercase tracking-wide mb-1">Service Type</label>
            ...
            <div>
              <label className="block text-xs font-bold text-ink uppercase tracking-wide mb-1">Logged Date</label>
          ...
          <div>
            <label className="block text-xs font-bold text-ink uppercase tracking-wide mb-1">Service Details & Notes</label>
            <textarea required rows={3} value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full px-3.5 py-2.5 border border-sage/35 rounded-xl focus:outline-none focus:ring-2 focus:ring-accent bg-white text-ink font-medium placeholder-sage/60" placeholder="Describe the repairs, issues, or details..."></textarea>
          </div>

          <div>
            <label className="block text-xs font-bold text-ink uppercase tracking-wide mb-1">Estimated Cost ($)</label>
            <input required type="number" min="0" step="0.01" value={formData.cost} onChange={e => setFormData({...formData, cost: parseFloat(e.target.value) || 0})} className="w-full px-3.5 py-2.5 border border-sage/35 rounded-xl focus:outline-none focus:ring-2 focus:ring-accent bg-white text-ink font-medium" />
          </div>
