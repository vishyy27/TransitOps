...
function DriverDrawer({ driverId, onClose }: { driverId: string; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 bg-slate-950/30 backdrop-blur-sm" onClick={onClose}><aside onClick={event => event.stopPropagation()} className="absolute right-0 top-0 h-full w-full max-w-md overflow-y-auto bg-white dark:bg-slate-900 p-6 shadow-2xl sm:p-8 border-l dark:border-slate-800"><div className="flex justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400 dark:text-slate-500">Driver profile</p><h2 className="mt-1 font-display text-2xl font-bold text-slate-900 dark:text-slate-100">{driver.name}</h2><p className="text-sm text-slate-500 dark:text-slate-400">{driver.contactNumber}</p></div><button onClick={onClose} className="rounded-full p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer"><X className="w-5 h-5" /></button></div><div className="mt-5 flex gap-2 items-center"><Badge status={driver.status} /><span className={cn('rounded-full px-2.5 py-1 text-xs font-bold', expired ? 'bg-rose-50 text-rose-600 dark:bg-rose-950/40 dark:text-rose-450' : 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950/40 dark:text-emerald-450')}>{expired ? 'License expired' : 'License valid'}</span>{expired && <button onClick={() => toast.success(`License renewal email reminder dispatched to ${driver.name} successfully!`)} className="ml-auto text-xs font-bold text-indigo-600 hover:text-indigo-850 bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-950/40 dark:text-indigo-400 px-3 py-1 rounded-full cursor-pointer transition-colors">Send Email Alert</button>}</div><div className="mt-7 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-slate-50 dark:bg-slate-800 p-4"><p className="text-[10px] font-bold uppercase text-slate-400 dark:text-slate-500">Safety score</p><p className="mt-1 font-mono text-xl font-bold text-slate-900 dark:text-slate-100">{driver.safetyScore}</p></div><div className="rounded-2xl bg-slate-50 dark:bg-slate-800 p-4"><p className="text-[10px] font-bold uppercase text-slate-400 dark:text-slate-500">License</p><p className="mt-1 text-sm font-bold text-slate-900 dark:text-slate-100">{driver.licenseCategory} · {driver.licenseNumber}</p><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Expires {new Date(driver.licenseExpiryDate).toLocaleDateString()}</p></div></div><section className="mt-8"><h3 className="font-display font-bold text-slate-900 dark:text-slate-100">Trip history</h3><div className="mt-4 space-y-3">{trips.length ? trips.map(trip => <div key={trip.id} className="rounded-2xl border border-slate-100 dark:border-slate-850 bg-slate-50 dark:bg-slate-800/40 p-4"><div className="flex justify-between gap-2"><p className="text-sm font-bold text-slate-900 dark:text-slate-100">{trip.source} → {trip.destination}</p><Badge status={trip.status} /></div><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{trip.plannedDistance} km · {trip.cargoWeight} kg cargo</p></div>) : <p className="rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 p-5 text-sm text-slate-500 dark:text-slate-400">No trip activity recorded yet.</p>}</div></section></aside></div>;
}
